	...
	_In_reads_(NumSubresources) const D3D12_SUBRESOURCE_DATA* pSrcData)
{
	...
	BYTE* pData;
	// 获取upload buffer资源中与指定索引子资源映射的指针，已方便将数据考入
	HRESULT hr = pIntermediate->Map(0,					// 子资源索引
		NULL,											// 描述要映射的内存范围信息Null表示整个范围
		reinterpret_cast<void**>(&pData));				// 一个接受资源数据的地址
	if (FAILED(hr))
	{
		return 0;
	}
	...
	{
		// 将upload缓冲区资源拷贝到供渲染流水线使用的顶点/索引缓冲区
...
template <UINT MaxSubresources>		// MaxSubresources表示需要分配的资源缓冲区中包含的子资源个数(eg:多个模型合并到一个缓冲区)
inline UINT64 UpdateSubresources(
	_In_ ID3D12GraphicsCommandList* pCmdList,
	_In_ ID3D12Resource* pDestinationResource,      // 要拷贝到的目标资源(也就是最终渲染流水线使用的顶点/索引缓冲区)
	_In_ ID3D12Resource* pIntermediate,             // 中间过渡资源(也就是上传堆缓冲区upload buffer)
	UINT64 IntermediateOffset,
	...
	_In_reads_(NumSubresources) D3D12_SUBRESOURCE_DATA* pSrcData)
{
	...
	ID3D12Device* pDevice;
	pDestinationResource->GetDevice(__uuidof(*pDevice), reinterpret_cast<void**>(&pDevice));
	// 在上传堆缓冲区(upload buffer)中分配空间并返回待复制资源的布局信息
	pDevice->GetCopyableFootprints(&Desc,		// 目标资源描述信息
		FirstSubresource,						// 该缓冲区存储资源中第一个子资源的索引(eg:将多个模型合并时一个缓冲区可能存在多份子资源)
		NumSubresources,						// 该缓冲区存储资源中包含子资源的个数
		IntermediateOffset,						// 资源偏移量
		Layouts,								// 返回每个子资源的描述信息
		NumRows,								// 记录每个子资源行数
		RowSizesInBytes,						// 记录每个子资源大小(eg:tex2D资源行数(NumRows)为32 每个像素为4个字节 则此时此值为128)
		&RequiredSize);							// 已字节为单位的填充总大小
	pDevice->Release();
